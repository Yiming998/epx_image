...
import { collection, getDocs, addDoc, updateDoc, deleteDoc, doc, writeBatch, query, orderBy, onSnapshot } from "firebase/firestore";
...
export default function Admin() {
  ...
  const fetchTemplates = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, "templates"), orderBy("sort_order", "asc"));
  ...
  const handleSave = async () => {
    try {
      ...
      const data = {
        ...
        sort_order: editingTemplate ? editingTemplate.sort_order : templates.length + 1,
      ...
      showToast(error.message || "保存时发生错误，请查看控制台");
    }
  };
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    try {
      ...
      items.forEach((item, index) => {
        const templateRef = doc(db, "templates", item.id);
        batch.update(templateRef, { sort_order: index + 1 });
      });
      await batch.commit();
    } catch (error) {
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          {auth.currentUser?.email !== "xym8866668@gmail.com" && (
            <p className="mt-2 text-red-600 font-medium">
              当前登录账号 ({auth.currentUser?.email}) 不是管理员，无法保存或删除模版。请使用管理员账号登录。
            </p>
          )}
        </div>
